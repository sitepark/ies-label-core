...
public final class SearchLabelsUseCase {
  ...
  @Inject
  SearchLabelsUseCase(LabelRepository repository, AuthorizationService accessControl) {
    this.repository = repository;
    this.accessControl = accessControl;
  }

  public List<Label> searchLabels(SearchLabelsRequest request) {

    if (!this.accessControl.isLabelReadable()) {
      throw new AccessDeniedException("Not allowed to read labels");
    }

    return this.repository.search(request.term(), request.scopes());
  }
}
